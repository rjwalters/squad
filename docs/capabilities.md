...
| Shared steward | `squad_steward_status`, `squad_steward_tick` | `squad steward status`, `squad steward tick` | Any peer reads durable bank/review/map state. Only configured steward ticks: atomic bounded reminder ledger; no automatic banking, approval or claim deletion. |
| Room drift report | `squad_room_doctor` | `squad doctor --room` | Read-only evidence-first report over unbanked work, integration/outline divergence, overdue/missing reviews, claim hygiene, and chat "banked" claims checked against the verified ledger. Distinguishes verified-clean, observed-unbanked, unreachable and unobserved declared-artifact branches. Any identity; never mutates the room. |
| Generated outline | `squad_outline_render`, `squad_outline_status`, `squad_outline_publish` | `squad outline render`, `squad outline status [path]`, `squad outline publish <request-key> [path]` | Same deterministic snapshot, exact bank/review provenance, owned-file protection and verified publication pipeline. Render/status are pure reads. |
...
| Re-entry               | Shared join workflow plus runtime-specific launch support                                                                         | `squad codex-reentry`                                                                                                              | Codex uses the foreground supervisor; Claude's opt-in Stop hook is installed with `--reentry`. Shared policy: 5m claims, 30–45m idle, observed mentions/reviews first; TTL, lifetime cap, stop and failure bounds. Pull-only: Codex polls ≤45s while waiting; Claude may emit intermediate model turns. No stopped-process push wake.                                                            |
