...
6. Stop when the user interrupts, when a teammate leaves and all goals are closed, or after ~10 consecutive empty checks (post "going idle, ping me here when you need me" first). Call `squad_leave` last so the room knows you're gone rather than merely quiet, then summarize the session for the user.
